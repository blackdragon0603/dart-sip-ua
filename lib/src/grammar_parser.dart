...
class GrammarParser {
  static final List<String> _ascii =
      List<String>.generate(128, (c) => new String.fromCharCode(c));
  ...
  List<Map<int, List>> _cache = List<Map<int, List>>.filled(252, <int, List>{});

  List<int> _cachePos = List<int>.filled(252, -1);

  List<bool> _cacheable = List<bool>.filled(252, false);

  int _ch = -1;

  int _cursor = 0;

  List<GrammarParserError> _errors = <GrammarParserError>[];

  List<String?> _expected = <String>[];
  ...
  void _failure([List<String?>? expected]) {
    ...
    if (token != null && tokenStart != null) {
      var alias = _tokenAliases[token];
      var flag = _tokenFlags[token];
      var name = _tokenNames[token];
      if (_failurePos > tokenStart &&
          _failurePos == _inputLen &&
          (flag & 1) != 0) {
        var message = "Unterminated '$name'";
        _errors.add(new GrammarParserError(
            GrammarParserError.UNTERMINATED, _failurePos, tokenStart, message));
        _expected.addAll(expected ?? [null]);
      } else if (_failurePos > tokenStart && (flag & 1) != 0) {
        var message = "Malformed '$name'";
        _errors.add(new GrammarParserError(
            GrammarParserError.MALFORMED, _failurePos, tokenStart, message));
        _expected.addAll(expected ?? [null]);
      } else {
        _expected.add(alias);
      }
  ...
  List _list(Object first, List next) {
    ...
    var list = List<dynamic>.filled(length + 1, null);
    list[0] = first;
    for (var i = 0; i < length; i++) {
      list[i + 1] = next[i][1];
    }
    ...
    switch (_ch >= 0 && _ch <= 1114111
      ...
      case 2:
        ...
        while (true) {
          $$ = _parse_SWS();
          if (!success) break;
          var seq = List<dynamic>.filled(3, null)..[0] = $$;
          $$ = _matchChar(58, ':');
          if (!success) break;
          seq[1] = $$;
          $$ = _parse_SWS();
    ...
    switch (_ch >= 0 && _ch <= 57
      ...
      case 0:
        ...
        while (true) {
          $$ = _parse_dec_octet();
          if (!success) break;
          var seq = List<dynamic>.filled(7, null)..[0] = $$;
          $$ = _matchChar(46, '.');
          if (!success) break;
          seq[1] = $$;
          $$ = _parse_dec_octet();
          ...
          case 0:
            while (true) {
              ...
              while (true) {
                $$ = _parse_h16();
                if (!success) break;
                var seq = List<dynamic>.filled(13, null)..[0] = $$;
                $$ = _matchChar(58, ':');
                if (!success) break;
                seq[1] = $$;
                $$ = _parse_h16();
              ...
              while (true) {
                $$ = _parse_h16();
                if (!success) break;
                var seq = List<dynamic>.filled(11, null)..[0] = $$;
              ...
              while (true) {
                $$ = _parse_h16();
                if (!success) break;
                var seq = List<dynamic>.filled(10, null)..[0] = $$;
                var testing0 = _testing;
                _testing = _cursor;
                switch (_ch >= 0 && _ch <= 1114111
                    ? 0
                  ...
                  case 2:
                    ...
                    while (true) {
                      $$ = _matchChar(58, ':');
                      if (!success) break;
                      var seq = List<dynamic>.filled(2, null)..[0] = $$;
                      $$ = _parse_h16();
                      if (!success) break;
                      seq[1] = $$;
                      $$ = seq;
              ...
              while (true) {
                $$ = _parse_h16();
                if (!success) break;
                var seq = List<dynamic>.filled(9, null)..[0] = $$;
              ...
              while (true) {
                $$ = _parse_h16();
                if (!success) break;
                var seq = List<dynamic>.filled(8, null)..[0] = $$;
          ...
          case 1:
            while (true) {
              ...
              while (true) {
                $$ = _matchString(_strings3, '::', false);
                if (!success) break;
                var seq = List<dynamic>.filled(12, null)..[0] = $$;
                $$ = _parse_h16();
                if (!success) break;
                seq[1] = $$;
                $$ = _matchChar(58, ':');
              ...
              while (true) {
                $$ = _matchString(_strings3, '::', false);
                if (!success) break;
                var seq = List<dynamic>.filled(6, null)..[0] = $$;
                $$ = _parse_h16();
                if (!success) break;
                seq[1] = $$;
                $$ = _matchChar(58, ':');
              ...
              while (true) {
                $$ = _matchString(_strings3, '::', false);
                if (!success) break;
                var seq = List<dynamic>.filled(4, null)..[0] = $$;
                $$ = _parse_h16();
                if (!success) break;
                seq[1] = $$;
                $$ = _matchChar(58, ':');
    ...
    switch (_getState(_transitions13)) {
      ...
      case 2:
        ...
        while (true) {
          $$ = _parse_Method();
          if (!success) break;
          var seq = List<dynamic>.filled(5, null)..[0] = $$;
          $$ = _parse_SP();
          if (!success) break;
          seq[1] = $$;
          $$ = _parse_Request_URI();
        ...
        while (true) {
          ...
          if (success) {
            ...
            var pos0 = _startPos, offset = $start;
            {
              ...
              var header;
              try {
                final data_user = data.user;
                final data_host = data.host;
                if (data_host != null && data_user != null) {
                  data.uri = URI(data.scheme, data_user, data_host, data.port,
        ...
        while (true) {
          ...
          if (success) {
            ...
            var pos0 = _startPos, offset = $start;
            {
              ...
              try {
                final data_uri = data.uri;
                if (data_uri != null) {
                  header =
                      NameAddrHeader(data_uri, data.display_name, data.params);
                }
                data.uri = null;
  ...
  static List<bool> _unmap(List<int> mapping) {
    ...
    var result = List<bool>.filled(length * 31, false);
    var offset = 0;
    for (var i = 0; i < length; i++) {
      var v = mapping[i];
      for (var j = 0; j < 31; j++) {
  ...
  void reset(int pos) {
    ...
    _cursor = pos;
    _cache = List<Map<int, List>>.filled(252, <int, List>{});
    _cachePos = List<int>.filled(252, -1);
    _cacheable = List<bool>.filled(252, false);
    _ch = -1;
  ...
  parse(input, startRule) {
    ...
    return parseFunctions[startRule]?.call();
  }
}
